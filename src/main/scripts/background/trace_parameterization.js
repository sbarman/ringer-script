function ParameterizedTrace(trace){
  ...
  this.parameterizeTypedString = function(parameter_name, original_string){
    console.log("parameterizing string");
    var curr_string = "";
    var char_indexes = [];
    ...
    for (var i = 0; i< trace.length; i++){
      ...
      if (_.contains(["keydown", "keypress", "keyup", "input", "textInput"], event_data["type"])){
        ...
        (function(){
          var new_char_indexes = char_indexes.slice();
          var new_curr_string = curr_string;
          toProcessLater.push(function(){processString(parameter_name, original_string, new_curr_string, new_char_indexes);});
          curr_string_chars = [];
        })(); //faking block scope
      }
    }
    console.log("toProcessLater",toProcessLater);
    for (var i = toProcessLater.length-1; i>=0; i--){
      var func = toProcessLater[i];
      func();
    }
  };
  
  function processString(parameter_name, original_string, string, char_indexes){
    console.log("processString", parameter_name, original_string, string, char_indexes);
